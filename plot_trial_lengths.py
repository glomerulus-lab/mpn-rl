...
def main():
    ...
    for name, cfg in AGENTS.items():
        ...
        data[name] = by_type
        print(f"  Go trials:   n={len(by_type[1])}  mean={np.mean(by_type[1]):.1f}  "
              f"std={np.std(by_type[1]):.1f}")
        print(f"  Nogo trials: n={len(by_type[0])}  mean={np.mean(by_type[0]):.1f}  "
    ...
    for name, cfg in AGENTS.items():
        vals = data[name][0] + data[name][1]
        vp = ax2.violinplot(vals, positions=[pos], widths=0.6,
        ...
        ax2.text(pos, np.mean(vals) + 0.3, f"{np.mean(vals):.1f}",
    ...
    ax2.set_title("Trial length distribution (all trials)", fontsize=12, fontweight="bold")
