...
def compute_returns(rewards: torch.Tensor, dones: torch.Tensor,
                    bootstrap_value: torch.Tensor, gamma: float) -> torch.Tensor:
...
class ActorCriticNet(nn.Module):
    ...
    def __init__(self, input_dim: int, action_dim: int, hidden_dim: int = 128,
                 core_type: str = 'lstm', activation: str = 'tanh',
                 lambda_max: float = 0.99, eta_init: float = 0.01,
                 lambda_init: float = 0.99, num_layers: int = 1,
                 mpn_bias: bool = True, lstm_forget_bias: float = 0.0):
        super().__init__()
        self.core_type = core_type
        self.num_layers = num_layers

        if core_type == 'rnn':
            self.core = nn.RNN(input_dim, hidden_dim, num_layers=num_layers, batch_first=True)
        elif core_type == 'lstm':
...
def train_neurogym(args):
    ...
    pbar = tqdm.tqdm(total=pbar_total, desc="Episodes" if use_ep_limit else "Frames", unit="ep" if use_ep_limit else "fr")

    episode = 0
    while (use_ep_limit and episode < args.num_episodes) or (not use_ep_limit and total_frames < args.total_frames):
        ...
        for step in range(args.max_episode_steps):
            ...
            chunk_entropies.append(-(policy_dist * torch.log(policy_dist + 1e-8)).sum(-1))
            ...
            if chunk_full or done or step == args.max_episode_steps - 1:
                ...
                returns = _compute_returns_episode(chunk_rewards, chunk_dones, next_value, args.gamma)
        ...
        if episode - last_eval_episode >= args.print_freq:
            ...
            eval_reward, eval_reward_std = _evaluate_actorcritic(
                model, make_train_env, args.num_eval_episodes,
                args.max_episode_steps, eval_seed, device,
            )
            ...
            if rolling_avg > best_rolling_avg:
                ...
                exp_manager.save_model(model, optimizer=optimizer,
...
def render_to_plot(args):
    ...
    device = torch.device('cpu')
    print(f"Model type: {config.get('model_type', 'lstm').upper()}, using CPU for rendering\n")
    ...
    env = _create_env_from_config(config)
    action_dim = env.action_space.n
    output_path = Path(args.output) if args.output else exp_manager.plot_dir / "episode_render.png"
    ...
    with torch.no_grad():
        ...
        h = None
        for _ in tqdm.tqdm(range(args.max_episode_steps), desc="Recording", unit="step"):
    ...
    axs[0].legend([f"Obs {i}" for i in range(observations.shape[1])],
                  loc='upper right', ncol=observations.shape[1])
...
def main():
    ...
    train_parser.add_argument('--normalize-advantages', action='store_true', default=False)
    ...
    train_parser.add_argument('--print-freq', type=int, default=50,
                              help='Evaluate and log every N episodes')
    ...
    train_parser.add_argument('--mpn-bias', action='store_true', default=True,
                              help='Use bias term in MPN hidden layer (default: True)')
    train_parser.add_argument('--no-mpn-bias', dest='mpn_bias', action='store_false',
