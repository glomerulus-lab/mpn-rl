...
class _ActorCriticNet(torch.nn.Module):
    ...
    def __init__(self, input_dim, action_dim, hidden_dim, num_layers,
                 activation, lambda_max, eta_init, lambda_init, mpn_bias,
...
def _load_model(exp_dir: Path, device: torch.device):
    ...
    ckpt = torch.load(exp_dir / 'checkpoints' / 'best_model.pt',
...
def _render_frame(all_episode_lengths, all_trial_types, layer_norms, layer_cmap, current_ep,
                  num_episodes, step_idx, step_data, num_layers, fig, axes, im_list,
                  ax_action, action_line, cursor_action,
                  ax_obs, obs_lines, cursor_obs, all_steps):
    ...
    for layer_idx, (ax, im) in enumerate(zip(axes, im_list)):
        ...
        im.set_data(rgba)
        ax.set_title(f'Layer {layer_idx}  ({M.shape[0]}×{M.shape[1]})',
                     fontsize=9, pad=3)
...
def make_gif(experiment_dir: str, num_episodes: int = 5,
             output_path: str = 'mpn_m_matrix.gif', fps: int = 3,
             device_str: str = 'cpu'):
    ...
    while len(all_episode_lengths) < num_episodes:
        ...
        if (is_go and need_go) or (not is_go and need_nogo) or seed > 50:
            ...
            print(f"  Episode {ep_num}: {len(steps)} steps  reward={total_reward:.2f}  [{trial_type}]")
    ...
    gs = gridspec.GridSpec(3, num_layers,
                           height_ratios=[4, 1, 1],
                           width_ratios=widths,
    ...
    for layer_idx, ax in enumerate(axes):
        ...
        ax.axis('off')
        ax.set_title(f'Layer {layer_idx}  ({M0.shape[0]}×{M0.shape[1]})',
                     fontsize=9, pad=3)
        im_list.append(im)
    ...
    # Observation timeline
    obs_lines = [
        ax_obs.step([], [], where='post', color=obs_colors[i % len(obs_colors)],
                    lw=1.5, label=obs_labels[i])[0]
    ...
    for step_idx, step_data in enumerate(all_steps):
        ...
        frame = _render_frame(
            all_episode_lengths, all_trial_types, layer_norms, layer_cmap, current_ep,
            num_episodes, step_idx, step_data, num_layers, fig, axes, im_list,
            ax_action, action_line, cursor_action,
            ax_obs, obs_lines, cursor_obs, all_steps,
        )
...
def main():
    ...
    parser.add_argument('--output', type=str,
                        default='figures/mpn_m_matrix_gonogo.gif')
