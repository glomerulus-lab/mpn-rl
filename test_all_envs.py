...
def test_env_training(env_name: str, model_type: str = 'lstm', frames: int = 1000) -> dict:
    ...
    try:
        ...
        result = subprocess.run(
            [
                sys.executable, 'main.py', 'train-neurogym',
                '--env-name', env_name,
                ...
                '--hidden-dim', '32',
                '--print-freq', '500',
                '--checkpoint-freq', '10000',  # Don't checkpoint
            ],
            capture_output=True,
            text=True,
            timeout=120,
        ...
        if success:
            return {'status': 'OK', 'output': result.stdout[-500:] if len(result.stdout) > 500 else result.stdout}
        else:
            return {'status': 'FAILED', 'error': result.stderr[-500:] if len(result.stderr) > 500 else result.stderr}
...
def main():
    ...
    parser.add_argument('--frames', type=int, default=1000, help='Frames for training test')
    args = parser.parse_args()

    envs_to_test = {args.env: PAPER_ENVS.get(args.env, args.env)} if args.env else PAPER_ENVS
